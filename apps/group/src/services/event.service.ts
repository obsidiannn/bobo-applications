...
export class EventService {
    ...
    static async broadcastRegister(userId: bigint, groupId: bigint, receiverId: bigint) {
        const instance = await EventService.make();
        return await instance.protoClient.broadcast(protos.event.EventType.GROUP_APPLY, {
            userId,
            groupId,
            receiverId
        });
